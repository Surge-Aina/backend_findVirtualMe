...
const QuoteRequest =
  mongoose.models.QuoteRequest || require("../../models/cleaningLady/QuoteRequest");
// ============================================
...
exports.getMyPortfolio = async (req, res) => {
  try {
    ...
    if (!portfolio) {
      ...
      // });
      // console.log(
      //   `✅ Added portfolio ${portfolio._id} to user ${userId}'s portfolios array`
      // );
    }
  ...
  try {
    ...
    if (portfolio) {
      ...
      if (req.user && req.user.id) {
        ...
        console.log(
          `✅ Added portfolio ${portfolio._id} to user ${req.user.id}'s portfolios array`
        );
      }
...
exports.newPortfolio = async (req, res) => {
  try {
    ...
    //     req.user.id,
    //     {
    //       $addToSet: { portfolios: portfolioEntry },
    //     },
    //     { new: true }
    //   );

    //   console.log(
    //     `✅ Added portfolio ${portfolio._id} (Type: ${portfolio.portfolioType}) to user ${req.user.id}'s portfolios array`
    //   );
    // }
    ...
    if (error.code === 11000) {
      console.error(
        "Slug already taken error in cleaning lady controller newPortfolio():",
        error
      );
      return res.status(400).json({
        message: "Slug already taken. Please choose a different one.",
    ...
    console.error(
      "Error creating new portfolio in cleaning lady controller newPortfolio():",
      error
    );
    res.status(500).json({ message: "Error creating new portfolio" });
  }
};
...
exports.addService = async (req, res) => {
  try {
    ...
    if (!portfolio) {
      return res
        .status(404)
        .json({ message: "Portfolio not found. Create a portfolio first." });
    }
...
exports.submitQuote = async (req, res) => {
  try {
    ...
    const portfolio = await Portfolio.findById(portfolioId).populate("userId", "email");
