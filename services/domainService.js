...
const domainService = {
    ...
    getDomain: async (req, res) => {
      ...
      try {
        ...
        const url = `${process.env.NAMECHEAP_URL}` +
        ...
        parser.parseString(xmlResponse, (err, result) => {
            ...
            const domainCheckResult = JSON.parse(jsonString).ApiResponse.CommandResponse[0].DomainCheckResult[0].$;
            const cleanResult = {
                domain: domainCheckResult.Domain,
                available: domainCheckResult.Available === 'true',
                isPremium: domainCheckResult.IsPremiumName === 'true',
                premiumPrice: parseFloat(domainCheckResult.PremiumRegistrationPrice),
